...
INPUT_DIR="output_bowtie2_FR/"
OUTPUT_DIR="clean_reads_FR"
mkdir -p "$OUTPUT_DIR"
...
# sort bam file
samtools sort -n -m 5G -@ 5 "$OUTPUT_DIR/${BASENAME}_filtered.bam" -o "$OUTPUT_DIR/${BASENAME}_filtered_sorted.bam"


  # Convert BAM to FASTQ
  samtools fastq -@ 8 -1 "$OUTPUT_DIR/${BASENAME}_R1.fastq.gz" -2 "$OUTPUT_DIR/${BASENAME}_R2.fastq.gz" "$OUTPUT_DIR/${BASENAME}_filtered_sorted.bam" -0 /dev/null -s /dev/null

  # Remove intermediate file (optional)
  rm "$OUTPUT_DIR/${BASENAME}_filtered.bam"
 rm "$OUTPUT_DIR/${BASENAME}_filtered_sorted.bam"
done
