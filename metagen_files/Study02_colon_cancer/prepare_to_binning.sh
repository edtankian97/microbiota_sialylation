...
CONTIGS_DIR="./megahit_outputs_colon_china"
OUT_PATH="bowtie_index_china_colon_paired"

mkdir -p "$OUT_PATH"

for R1 in $READS_DIR/*.clean.trim.rmhost.1.fq.gz; do

    # Find mate R2
    R2="${R1/.clean.trim.rmhost.1.fq.gz/.clean.trim.rmhost.2.fq.gz}"

    # Name of sample (ex.: CRR224755)
    BASENAME=$(basename "$R1" .clean.trim.rmhost.1.fq.gz)
